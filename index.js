...
mongoose.connect("mongodb+srv://satish:mongodb374@cluster0.ltudgvf.mongodb.net/booksDatabase?retryWrites=true&w=majority").then(() => {
...
class Library {
    ...
    async addBook(book) {
        try {
            ...
            await newBook.save();
            return `Book: '${book.title}' is added to the library.`;
        } catch (error) {
        ...
        try {
            ...
            if (books.length === 0) {
                ...
                return books.map(book =>
                    book.fileFormat ? new EBook(book.title, book.author, book.ISBN, book.fileFormat).displayInfo()
                    : new Book(book.title, book.author, book.ISBN).displayInfo())
            }
...
app.get("/book/:title", async(req, res) => {
    try{
        ...
        const bookObj = await library.searchByTitle(title);
